...
500.times do |n|
    country = countries[rand(0..100)][1]
    params = { :member => {
        :email => "example-#{n+1000}@example.org",
        :password => "fidelio",
    ...
    user = member.user
      ...
      schedule = [ "Daily", "Weekly", "Fortnightly", "Monthly", "Every 2 months", "Every 3 months", "Less frequently than every 3 months"]
        ...
        user.institute_type = "University College Hospital"
        user.country = country
        user.nonild =10
        user.ipfpatients = 20
        user.hppatients = 30
        user.sarcoidpatients = 10
        user.iippatients = 10
        user.ctdpatients = 10
        user.unclasspatients = 10
        user.institute = "ILD institute"
        user.schedule = schedule[rand(0..6)]
        user.ipf = rand(0..40)
        user.cases = rand(0..100)
        user.save
        p = Physician.new
        p.user_id  = user.id
        p.experience = 10
        p.kind = "Pulmonologist (fellowship trained)"
        p.save
